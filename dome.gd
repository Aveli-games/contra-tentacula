...
func _ready():
	...
	$Building/InfestationProgress.value = 0
...
func _process(delta):
	...
	var progress_diff = infestation_percentage - $Building/InfestationProgress.value/100
	...
	if progress_adjustment != 0:
		$Building/InfestationProgress.value += progress_adjustment * 100
